...
fn monocle_toggle_sets_monocle_window() {
    ...
    mon.active_ws_mut().set_monocle(true);
    mon.active_ws_mut().set_monocle_window(focused_window);

    assert!(mon.active_ws().monocle());
    assert_eq!(mon.active_ws().monocle_window(), Some(100));
}
...
fn monocle_toggle_clears_monocle_window() {
    let mut mon = make_monitor(1);
    mon.active_ws_mut().set_monocle(true);
    mon.active_ws_mut().set_monocle_window(Some(100));

    // Simulate toggle_monocle: disable
    mon.active_ws_mut().set_monocle(false);
    mon.active_ws_mut().set_monocle_window(None);

    assert!(!mon.active_ws().monocle());
    assert_eq!(mon.active_ws().monocle_window(), None);
}
...
fn monocle_entry_uses_monocle_window() {
    ...
    mon.active_ws_mut().set_monocle(true);
    mon.active_ws_mut().set_monocle_window(Some(200));
...
fn monocle_clears_when_monocle_window_destroyed() {
    ...
    if mon.active_ws().monocle() && mon.active_ws().monocle_window() == Some(100) {
        mon.active_ws_mut().set_monocle(false);
        mon.active_ws_mut().set_monocle_window(None);
    }
...
fn monocle_persists_when_other_window_destroyed() {
    ...
    if mon.active_ws().monocle() && mon.active_ws().monocle_window() == Some(200) {
        mon.active_ws_mut().set_monocle(false);
        mon.active_ws_mut().set_monocle_window(None);
    }
